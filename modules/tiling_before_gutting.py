...
def convert_bin_columns_to_las(bin_dir, las_dir, template_header):
    ...
    for col, files in files_by_column.items():
        ...
        if crs:
            wkt = crs.to_wkt("WKT2_2019") + "\x00"
            evlr = WktCoordinateSystemVlr(wkt.encode("utf-8"))
            header.evlrs = VLRList([evlr])
        ...
        with laspy.open(out_path, mode="w", header=header) as writer:
            if raw_data:
                ...
                X = point_record.X * header.x_scale + header.x_offset
                Y = point_record.Y * header.y_scale + header.y_offset
                Z = point_record.Z * header.z_scale + header.z_offset
                logger.info(f"🔍 Column {col} X:[{X.min()}–{X.max()}] Y:[{Y.min()}–{Y.max()}] Z:[{Z.min()}–{Z.max()}]")
